...
int compare(int a, int b) {     
	// Compare 2 nos without if-else statements, >, <, == and - operators.
    return sign(a + (~b + 1));
}

int sign(int x) {               
	// Give sign info about a no: 1 if x>0, -1 if x<0, 0 if x=0.
    return (x>>31) | (!!x);
}

int bitAnd(int a, int b) {      
	// Bitwise AND using ~ and | only.
    return ~(~a | ~b);
}

int bitXor(int a, int b) {      
	// Bitwise XOR using ~ and & only.
    return ~(~(~a&b) & ~(a&~b));
}

int not(int x) {                
	// Return !x without using ! operator.
    int neg_x = ~x + 1;
...
int conditional(int a, int b, int c) {  
	// Implement conditional operator ?: using logical operators.
...
int invertRange(int a, int p1, int p2) {    
	// Invert bits in a from position p1 to p2.

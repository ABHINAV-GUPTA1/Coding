...
/* 
 * The central sorting function which can take different comparison functions
 * as argument via function pointer 
 */
    ...
    for(i = 1; i < argc - 1; i++)
        for(j = 1; j < argc - 1; j++)
            if((*compare)(argv[j], argv[j+1]) > 0) {
				// Make comparison according to the passed comparison function pointer
